...
def generate_daily_report(**context):
    ...
    try:
        ...
        if opensearch_client.health_check():
            try:
                stats_response = opensearch_client.client.indices.stats(index=opensearch_client.chunk_index_name)
                count_response = opensearch_client.client.count(index=opensearch_client.chunk_index_name)

                index_stats = stats_response["indices"][opensearch_client.chunk_index_name]["total"]
                ...
                logger.error("Failed to get OpenSearch statistics: %s", stats_error)
                report["opensearch_statistics"] = {
                    "index_name": opensearch_client.chunk_index_name,
